...
public class CategoryDataService : BaseDataService, ICategoryDataService
{
    ...
    public CategoryDataService(IClient client, IMapper mapper, ILocalStorageService localStorage) : base(client,
        localStorage)
    {
        _mapper = mapper;
    }
    ...
    public async Task<ApiResponse<CategoryDto>> CreateCategory(CategoryViewModel categoryViewModel)
    {
        try
        {
            ...
            var createCategoryCommand = _mapper.Map<CreateCategoryCommand>(categoryViewModel);
